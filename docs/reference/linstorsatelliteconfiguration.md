...
*   Setting no `nameTemplate` will create one property using `name`. The value of the property is the
    joined expansion of the `valueTemplate` field for every matched field. See above for supported expansions. The
    result is joined using the optional `delimiter` value.
